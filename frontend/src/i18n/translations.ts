...
const DICT = {
  ...
  'simpleMode.help': {
    en: 'Check a popular service below, or search for any other one — results are pulled from SagerNet\'s geosite rule sets, runetfreedom\'s independently maintained mirror of the same categories, and the vernette/rulesets collection (extra AI services, plus Discord/Telegram variants that also cover voice-chat IP ranges). The source is shown next to each result — pick whichever mirror you trust more for a given category, or add both. Picking a service with its own desktop app (Discord, Telegram, ...) also routes that app\'s executable through the proxy automatically. Need finer control (domains, ports, process names, ...)? Switch to Advanced above.',
